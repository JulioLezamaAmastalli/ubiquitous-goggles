...
sql_h2h_model_path=os.path.join(path, 'sql/create_h2h_model_predictions.sql')
sql_insert_model=os.path.join(path, 'sql/insert_h2h_model_predictions.sql')
...
###############################################
## DATA REQUEST TO DB: MATCHES FROM LAST 7 DAYS
############################################### 
...
data_request_string='SELECT * FROM h2h.source_predictions WHERE (match_day BETWEEN ' +between+ ')'
...
# Create predicted Y variable (it will later on be filled up when we make predictions)
model_data['probs'] = ""

# Filter only columns for model
model_data=model_data[["probs","league_id","season_id","venue_id","referee_id","localteam_id",'visitorteam_id',"localteam_position","visitorteam_position","match_day"]]
