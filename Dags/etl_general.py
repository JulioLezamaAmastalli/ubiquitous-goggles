...
sql_h2h_general_path=os.path.join(path, 'sql/create_h2h_general.sql')
sql_insert_general=os.path.join(path, 'sql/insert_h2h_general.sql')
...
with open(sql_h2h_general_path) as ddl:
    cursor.execute(ddl.read())
...
for value in general_values:
    with open(sql_insert_general) as dml:
        try:
