...
sql_h2h_scores_path=os.path.join(path, 'sql/create_h2h_scores.sql')
sql_insert_scores=os.path.join(path, 'sql/insert_h2h_scores.sql')
...
with open(sql_h2h_scores_path) as ddl:
    cursor.execute(ddl.read())
...
for value in scores_values:
    with open(sql_insert_scores) as dml:
        try:
