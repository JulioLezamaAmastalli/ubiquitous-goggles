...
sql_h2h_standings_path=os.path.join(path, 'sql/create_h2h_standings.sql')
sql_insert_standings=os.path.join(path, 'sql/insert_h2h_standings.sql')
...
with open(sql_h2h_standings_path) as ddl:
    cursor.execute(ddl.read())
...
for value in standings_values:
    with open(sql_insert_standings) as dml:
        try:
